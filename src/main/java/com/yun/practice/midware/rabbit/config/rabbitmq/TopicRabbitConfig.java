package com.yun.practice.midware.rabbit.config.rabbitmq;
...
public class TopicRabbitConfig {
    ...
    public Queue topicFirstQueue(){
        ...
        return new Queue(MessageQueueConstant.FIRST_QUEUE,true);
    }
    ...
    public Queue topicSecondQueue(){
        ...
        // autoDelete:是否自动删除，有消息者订阅本队列，然后所有消费者都解除订阅此队列，会自动删除。
        // arguments：队列携带的参数，比如设置队列的死信队列，消息的过期时间等等。
        return new Queue(MessageQueueConstant.SECOND_QUEUE,true);
    }
    ...
    TopicExchange testTopicExchange(){
        ...
        return new TopicExchange(MessageQueueConstant.TEST_EXCHANGE,true,false);
    }
    //绑定交换机和队列
    @Bean
    Binding bindingFirstTopic(){
        //bind队列to交换机中with路由key（routing key）
        return BindingBuilder.bind(topicFirstQueue()).to(testTopicExchange()).with(MessageQueueConstant.FIRST_ROUTINGKEY);
    }

    @Bean
    Binding bindingSecondTopic(){
        //bind队列to交换机中with路由key（routing key）
        return BindingBuilder.bind(topicSecondQueue()).to(testTopicExchange()).with(MessageQueueConstant.SECOND_ROUTINGKEY);
    }
